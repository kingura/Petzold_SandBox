...
class LineCaps : PrintableForm
{
  ...
  protected override void DoPage(Graphics grfx, Color clr, int cx, int cy)
  {
    ...
    {
      grfx.DrawString(lc.ToString(), Font, brush, Font.Height, Font.Height / 2);

      penWide.StartCap = lc;
      penWide.EndCap = lc;

      grfx.DrawLine(penWide, 2 * cx / 4, Font.Height, 3 * cx / 4, Font.Height);
      grfx.DrawLine(penNarrow, 2 * cx / 4, Font.Height, 3 * cx / 4, Font.Height);
