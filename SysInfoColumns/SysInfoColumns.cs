...
internal class SysInfoColumns : Form
{
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        grfx.DrawString(SystemInformation.BootMode.ToString(), Font, brush, cxCol, y);
        y += cySpace;

        grfx.DrawString("Border3DSize", Font, brush, 0, y);
        grfx.DrawString(SystemInformation.Border3DSize.ToString(), Font, brush, cxCol, y);
        y += cySpace;

        grfx.DrawString("BorderSize", Font, brush, 0, y);
        grfx.DrawString(SystemInformation.BorderSize.ToString(), Font, brush, cxCol, y);
        y += cySpace;
        ...
        grfx.DrawString(SystemInformation.CaptionHeight.ToString(), Font, brush, cxCol, y);
        y += cySpace;

        grfx.DrawString("ComputerName", Font, brush, 0, y);
        grfx.DrawString(SystemInformation.ComputerName, Font, brush, cxCol, y);
        y += cySpace;

        grfx.DrawString("CursorSize", Font, brush, 0, y);
        grfx.DrawString(SystemInformation.CursorSize.ToString(), Font, brush, cxCol, y);
        y += cySpace;

        grfx.DrawString("DbcsEnabled", Font, brush, 0, y);
        grfx.DrawString(SystemInformation.DbcsEnabled.ToString(), Font, brush, cxCol, y);
    }
}